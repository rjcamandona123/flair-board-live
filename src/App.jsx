...
async function actionAddPin(form) {
  ...
  if (url) {
    ...
    try { var fr = await fetch('/favicon-resolver?url=' + encodeURIComponent(url)); var ft = await fr.text(); var fj = ft ? JSON.parse(ft) : {}; icon = fj.favicon || ('https://icons.duckduckgo.com/ip3/' + domain + '.ico'); urlTitle = fj.title || ''; if (!label) label = urlTitle || domain.replace(/^www\./, ''); } catch(e) { icon = 'https://icons.duckduckgo.com/ip3/' + domain + '.ico'; if (!label) label = domain.replace(/^www\./, ''); }
...
function renderUploadsPage(el) {
  ...
  el.innerHTML = renderNavbarHTML() + '<div class="page-content uploads-page">'
    ...
    + '<form class="create-board-form hidden" id="create-form"><label for="board-name" style="font-size:12px;color:#555">Board Name</label><input type="text" name="name" id="board-name" placeholder="Board name..." maxlength="50" required autocomplete="off"><button type="submit" class="btn-primary">Create</button><a href="#" class="btn-secondary" id="cancel-create">Cancel</a></form>'
...
function renderBoardPage(el) {
  ...
  var embedSection = board.is_public ? '<div class="embed-section"><h3 class="embed-heading">📋 Share / Embed this Board</h3><div class="embed-body"><label class="embed-label" for="public-url-input">Public URL:</label><div class="embed-copy-row"><input type="text" id="public-url-input" class="embed-copy-input" readonly value="' + escAttr(location.origin + location.pathname + '?route=public/' + bid) + '" spellcheck="false" autocomplete="off"><button class="embed-copy-btn" onclick="var i=this.previousElementSibling;i.select();navigator.clipboard.writeText(i.value);this.textContent=\'Copied!\';setTimeout(function(){this.textContent=\'Copy\'}.bind(this),2000)">Copy</button></div><label class="embed-label" for="embed-code-input">Embed iframe code:</label><div class="embed-copy-row"><input type="text" id="embed-code-input" class="embed-copy-input" readonly value=\'<iframe src="' + escAttr(location.origin + location.pathname + '?route=public/' + bid) + '" width="100%" height="600" frameborder="0"></iframe>\' spellcheck="false" autocomplete="off"><button class="embed-copy-btn" onclick="var i=this.previousElementSibling;i.select();navigator.clipboard.writeText(i.value);this.textContent=\'Copied!\';setTimeout(function(){this.textContent=\'Copy\'}.bind(this),2000)">Copy</button></div><p class="embed-note">Visitors can add their own pins and delete only the ones they added.</p></div><div class="embed-preview"><div class="embed-preview-frame"><iframe src="' + escAttr(location.origin + location.pathname + '?route=public/' + bid) + '" width="100%" height="450" frameborder="0"></iframe></div></div></div>' : '';
  el.innerHTML = renderNavbarHTML() + '<div class="page-content board-page" id="board-page">'
    ...
    + '<input type="color" id="pin-color-picker" style="position:fixed;top:-100px;left:-100px;width:0;height:0;opacity:0;pointer-events:none">'
      + '<div id="flair-picker" class="flair-picker hidden"><h3>Add a Pin</h3><form id="add-pin-form" novalidate style="display:flex;flex-wrap:wrap;gap:6px"><input type="hidden" name="board_id" value="' + bid + '"><input type="hidden" name="x" value="400"><input type="hidden" name="y" value="250"><label for="pin-url" style="font-size:12px;color:#555;align-self:center">URL</label><input type="url" name="url" id="pin-url" placeholder="URL" style="flex:2;min-width:140px" autocomplete="off"><input type="text" name="label" id="pin-label" placeholder="Text (optional)" style="flex:1;min-width:140px" autocomplete="off"><input type="file" name="image" accept="image/*" style="flex:1;min-width:100px"><input type="color" name="color" value="#cc3333" style="width:40px;height:36px;padding:2px"><input type="color" name="text_color" value="#ffffff" style="width:40px;height:36px;padding:2px"><button type="submit" class="btn-primary" style="padding:6px 14px;font-size:13px">Add Pin</button></form><button class="btn-secondary" style="display:block;width:100%;margin-top:8px;cursor:pointer" id="cancel-picker">Cancel</button></div>'
    + '</div>';
  bindBoardSVGs();
}
...
function renderPublicBoardPage(el) {
  ...
  el.innerHTML = '<div style="font-family:sans-serif;background:#e8e0d5;min-height:100vh;padding:10px">'
    ...
    + '<input type="hidden" name="board_id" value="' + bid + '">'
     ...
     + '<label for="pub-url" style="font-size:12px;color:#555;align-self:center">URL</label>'
     + '<input type="url" name="url" id="pub-url" placeholder="URL" style="flex:2;min-width:140px" autocomplete="off">'
     + '<input type="text" name="label" id="pub-label" placeholder="Text (optional)" style="flex:1;min-width:140px" autocomplete="off">'
